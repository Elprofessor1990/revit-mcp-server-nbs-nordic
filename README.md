...
#### Local NBS installer: writes and verification

`INSTALLA.bat` now uses this checkout's **prepared local Release builds**, never upstream release ZIPs. The distributor prepares the checkout with `npm ci` / `npm run build:local` in `server`, and `dotnet build` for **both** `plugin/RevitMCPPlugin.csproj` and `commandset/RevitMCPCommandSet.csproj`, with `-c "Release R27" -p:SkipAddinCopy=true -p:PublishAddinFiles=false` (matching the target year). Alternatively the build-machine operator can run `scripts/install.ps1 -Build -RevitVersion 2027`, which builds and then installs. Do not use Debug builds for packaging. Distribute the prepared checkout including `scripts`, `server/package.json`, `server/build`, `plugin/tool_schemas.json`, and complete `plugin/bin/AddIn <year> Release Rxx` outputs. The student runs the installer, opens Revit/their project, then opens their AI client; no npm/dotnet commands are needed on the student machine. Missing artifacts stop installation before Addins/config writes.

The original STEP 1–4 are unchanged, including the internet prerequisite, Node LTS installation choices, and legacy `-Uninstall` behavior. Close Revit before replacement; the new install phase refuses to copy while Revit is running. An existing, conflicting `revit-mcp-nbs` client entry is **not overwritten**; the installer reports that client separately and continues configuring the others. This is intentionally a review-required conflict, not permission to replace custom settings.

| Operation | Exact write scope / recovery |
| --- | --- |
| Optional original Node installer (STEP 4) | Downloads `%TEMP%/node-lts-installer.msi`, launches the interactive MSI (system Node install/PATH chosen by its wizard), then removes that MSI. Not rolled back by this script. |
| `-Build` only | Replaces `server/node_modules` via `npm ci`; writes `server/build`, `plugin/tool_schemas.json`, plugin/commandset `bin` and `obj`, and normal npm/NuGet caches. No Debug/live deployment. Requires matching .NET SDK. |
| Plugin deployment | `%APPDATA%/Autodesk/Revit/Addins/<year>/mcp-servers-for-revit.addin`; under `revit_mcp_plugin`: compiled DLL/PDB/deps/runtimeconfig files, `tool_schemas.json`, `Commands/RevitMCPCommandSet/<year>` binaries, `Commands/RevitMCPCommandSet/command.json`, and `Commands/RevitMCPCommandSet/server/{package.json,build/index.js,build/sql-wasm.wasm,build/cci-hierarchy.json}`. Creates `Commands/commandRegistry.json` **only if absent**. No folder deletion; no official NBS addin, existing registry, settings, port files or databases are replaced. |
| Deployment recovery | `<checkout>/artifacts/installer-backup-<year>-<GUID>/manifest.json` records every destination and whether it existed; prior files are copied alongside and hash-checked before replacement. Copy failure restores existing touched files and removes only newly created touched files; empty new directories may remain. Successful earlier years are not rolled back if a later year fails. Close Revit/clients before manual restore using this manifest; backups are not automatically pruned. |
| Claude Desktop | `%APPDATA%/Claude/claude_desktop_config.json`, or detected Store package `%LOCALAPPDATA%/Packages/Claude_*/LocalCache/Roaming/Claude/claude_desktop_config.json`; adds only `mcpServers.revit-mcp-nbs`. |
| Claude Code | `%USERPROFILE%/.claude.json` (or `%CLAUDE_CONFIG_DIR%/.claude.json`); adds only user-scoped `mcpServers.revit-mcp-nbs`. Other servers/project preferences are preserved. |
| Codex | `%CODEX_HOME%/config.toml`, otherwise `%USERPROFILE%/.codex/config.toml`; appends `[mcp_servers.revit-mcp-nbs]` with absolute Node/server paths. Existing text/comments remain byte-for-byte; ambiguous/conflicting tables are rejected. Format follows [Codex MCP documentation](https://developers.openai.com/codex/mcp). |
| Client recovery | Each replaced config gets a unique adjacent `.installer-<GUID>.bak`; a temporary `.installer-<GUID>.tmp` is atomically replaced/cleaned. Restore the backup to undo, or remove only the new entry if other client settings have changed since. A newly created config has no prior backup. JSON formatting can change, not unrelated values. Malformed JSON is rejected. |
| Final read-only verification | Starts the installed MCP server, performs `initialize`, `tools/list`, and `get_project_info` through the real bridge; waits up to 180 seconds for Revit/project (`-VerifyTimeoutSeconds 1..600`). No model mutations or NBS API calls. The server can initialize `%USERPROFILE%/.mcp-revit/revit-data.db` and append `revit_mcp_plugin/Commands/RevitMCPCommandSet/logs/token-usage.jsonl`; the plugin may append its existing logs. Tool-Agent is disabled for this probe. Timeout/error exits nonzero and **does not claim live success** or undo installation. |

Reviewer checks: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/test-install-local.ps1` (isolated real filesystem operations in unique `artifacts/installer-test-*` directories), `node --test scripts/test-verify-install.mjs` (stdio fixtures), and `node scripts/verify-install.mjs "<installed server>/build/index.js" 12` (real, non-mutating connection check). Fixture passes do not establish that DLLs load in Revit. A full first-time/replacement install and live successful round-trip still need review and a running Revit instance. **Legacy `-Uninstall` deletes installed plugin folders without backup and is not reversible**; it was deliberately left unchanged with STEP 1–4 and was not tested on this machine.

#### First-time installation layout
