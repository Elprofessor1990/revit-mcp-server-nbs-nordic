...
await build({ entryPoints: ["tests/agent/orchestrator.test.ts"], bundle: true,
  platform: "node", format: "esm", external: ["sql.js"],
  outfile: ".test-build/agent-orchestrator.test.mjs",
...
const result = spawnSync(process.execPath, ["--test", ".test-build/agent-orchestrator.test.mjs"], { stdio: "inherit" });
