...
import { readModelSettings, decodeNativeLinkType } from "../integrations/nbs/ModelSettings.js";
...
export function registerNbsProjectConnectionTools(server: McpServer) {
  server.tool("get_connection_status", "Read connection status, active Revit model identity, NBS project, missing parameter bindings, the sync defaults (link mode, fields) the user saved for this model in Revit Settings > NBS Nordic, and the official NBS addin's own Link Type (nativeLinkType code, decoded as nativeLinkMode). Never returns API keys.", {}, async () => {
    ...
    return rawToolResponse("get_connection_status", {
      ...
      // Decoded here as well so an older plugin build (raw code only) still reports the mode.
      nativeLinkMode: model ? decodeNativeLinkType(model.nativeLinkType) ?? null : null,
